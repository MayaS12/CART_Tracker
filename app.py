...
1. "Suggested next steps (draft — review before acting):" followed by a short bullet list of \
concrete things to consider doing (e.g. specific tests, monitoring, follow-up) — phrase each as \
a suggestion, not an order ("Consider...", "Worth checking...").
2. "Why:" followed by a short bullet list of the specific findings driving those suggestions — \
this is where flagged concerns and reasoning go, not before the plan.
3. One final line, always included, reminding the specialist this is an AI-drafted starting \
point that needs their own independent clinical judgment before anything is sent back to the \
nurse.

Rules:
- Work ONLY from the data given. Never invent a lab value, symptom, or history detail that \
isn't in the context below.
- Keep the whole thing under 130 words, not counting the three section headers and the final \
reminder line — brevity matters more than covering every possible point, and the response MUST \
end with the reminder line rather than running out of room before reaching it.
- If a vital or lab value looks physiologically implausible (e.g. an SpO2 or pulse far outside \
survivable range), say so explicitly as something to verify for a possible charting/device error, \
...
HLH suspicion, abnormal labs), it should be the first bullet under "Why", not buried.
"""
...
def get_ai_suggestion(entry):
    ...
    try:
        ...
        if choice.get("finish_reason") == "length":
            text += "\n\n[Cut off before finishing — token budget too small for this response. Treat as incomplete, not just brief.]"
        return text, None
