...
class Patient(db.Model):
    ...
    disease_status = db.Column(db.String(32), nullable=True)
    ...
    prior_therapy_lines = db.Column(db.Text)
    ...
    def latest_toxicities(self):
        all_entries = sorted(self.entries, key=lambda e: e.entry_date, reverse=True)
        result = {"crs_grade": None, "icans_grade": None, "hlh_suspected": None,
                  "igg": None, "infection_findings": None, "other_concern": None,
                  "fever_present": False, "as_of": None}
        for e in all_entries:
            ...
            if result["other_concern"] is None and e.other_toxicity_note:
                result["other_concern"] = e.other_toxicity_note
                result["as_of"] = result["as_of"] or e.entry_date
        return result
...
PATIENT_XLSX_COLUMNS = [
    ...
    "Latest CRS", "Latest ICANS", "Latest HLH suspected", "Latest IgG",
    "Latest infections", "Latest other",
]
VISIT_XLSX_COLUMNS = [
...
def _patient_xlsx_row(p):
    ...
    return [
        ...
        tox["crs_grade"], tox["icans_grade"], tox["hlh_suspected"], tox["igg"],
        tox["infection_findings"], tox["other_concern"],
    ]
