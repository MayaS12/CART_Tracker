...
app.config["MAX_CONTENT_LENGTH"] = 25 * 1024 * 1024  # 25MB per request — protects the free-tier instance
...
class Attachment(db.Model):
    """Photos/videos attached to an entry. Stored as bytes in the database
    (not on disk) so they survive Render free-tier restarts, which wipe the
...
def save_attachments(entry, files):
    saved, skipped = 0, []
    for f in files:
        if not f or not f.filename:
            continue
        content_type = f.content_type or ""
        if not content_type.startswith(ALLOWED_ATTACHMENT_TYPES):
            skipped.append(f.filename)
            continue
        db.session.add(Attachment(
            entry_id=entry.id, filename=f.filename, content_type=content_type,
...
def new_entry(patient_id):
    ...
    if request.method == "POST":
        ...
        if token:
            ...
            if existing:
                # Same form submitted twice (double-click, slow upload + retry, etc.)
                # — don't create a second entry, just send them to the first one.
        ...
        saved, skipped = save_attachments(e, request.files.getlist("attachments"))
        db.session.commit()
...
def delete_attachment(attachment_id):
    ...
    entry_id = a.entry_id
    patient_id = a.entry.patient_id
    db.session.delete(a)
    db.session.commit()
    flash("Attachment removed")
    # send back wherever they came from — timeline is the common case
