...
ROUTINE_VISIT_SYMPTOM_OPTIONS = [
    "fever", "headache", "dizziness", "confusion", "diarrhea", "vomiting",
]
...
class Patient(db.Model):
    ...
    def latest_toxicities(self):
        ...
        for e in approved:
            ...
            if not result["fever_present"] and "fever" in e.symptom_list():
...
def build_ai_context(entry):
    ...
    symptoms = entry.symptom_list()
    if symptoms or entry.symptoms_note:
        lines.append(f"Symptoms: {', '.join(symptoms) or '—'}"
                      + (f" — notes: {entry.symptoms_note}" if entry.symptoms_note else ""))
    labs = entry.lab_dict()
    if labs:
...
def new_entry(patient_id):
    ...
    return render_template(
        ...
        symptom_options=ROUTINE_VISIT_SYMPTOM_OPTIONS, lab_groups=LAB_GROUPS,
